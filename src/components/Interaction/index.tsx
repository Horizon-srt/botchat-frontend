...
import { ChatWithOpenAIProps, DetailProps, TopicProps } from '@/utils/appType';
...
const Interaction: React.FC<InteractionProps> = ({
  ...
  const chat = async (args: ChatWithOpenAIProps) => {
    try {
      const res_chat_with_ai = await postChatWithOpenAI(args);
      if (!res_chat_with_ai) {
        Message.error('No response from server... Please try again');
        return;
      }
      const res_chat_res_to_voice = await postTextToSpeech({
        conversation_id: res_chat_with_ai.conversation_id,
        response_word: res_chat_with_ai.response
      });
      if (!res_chat_res_to_voice) {
        Message.error('Get voice failed... Please try again');
        return;
      }
      // finish chat process
      setUpdateTopic(!updateTopic);
    } catch (e) {
      Message.error('Interact with server failed... Please try again');
    }
  };
  ...
  const handleWord = async () => {
    try {
      ...
      Message.error('Submit prompt failed... Please try again');
    }
  };
